      ...
      <div className="max-w-5xl mx-auto p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          {/* <div className="flex gap-3 text-blue-600">
            <Link className="underline" href="/">สุ่มเมนู</Link>
            <Link className="underline" href="/add">+ เพิ่มเมนู</Link>
          </div> */}
        </div>

        {/* Filters (เรียบง่าย/ไม่ซ้อน) */}
        <div className="rounded border bg-white p-4 space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {(["", "breakfast", "lunch", "dinner"] as (TimeKey | "")[]).map((t) => (
              <button
                ...
                type="button"
                onClick={() => setTime(t)}
                className={`px-3 py-1 rounded border ${time === t ? "bg-black text-white" : "bg-white"}`}
              >
          ...
          <div className="flex flex-wrap gap-2">
            {allTags.map((t) => (
              <button
                ...
                type="button"
                onClick={() => toggleTag(t)}
                className={`px-3 py-1 rounded border ${tags.includes(t) ? "bg-black text-white" : "bg-white"}`}
              >
                {t}
              </button>
            ))}
          ...
          <div className="flex gap-2">
            {/* <button onClick={load} className="px-4 py-2 rounded bg-gray-900 text-white">ค้นหา</button>
            <button onClick={() => { setQ(""); setTime(""); setTags([]); setTimeout(load, 0); }} className="px-4 py-2 rounded border">
              ล้างเงื่อนไข
            </button> */}
            <button onClick={() => { setPage(1); load(); }} className="px-4 py-2 rounded bg-gray-900 text-white">
                ค้นหา
            </button>
        ...
        <div className="mt-6 overflow-x-auto rounded border bg-white">
          {loading ? (
            <div className="p-4 text-gray-600">กำลังโหลด...</div>
          ) : error ? (
            <div className="p-4 text-rose-600">เกิดข้อผิดพลาด: {error}</div>
          ) : items.length === 0 ? (
            <div className="p-4 text-gray-600">ไม่พบรายการ</div>
          ) : (
            <table className="w-full table-auto">
              <thead className="bg-gray-100 text-left">
                <tr>
                  <th className="px-3 py-2 border-b">ชื่ออาหาร</th>
                  <th className="px-3 py-2 border-b">มื้อ</th>
                  <th className="px-3 py-2 border-b">แท็ก</th>
                  <th className="px-3 py-2 border-b">งบ</th>
                  <th className="px-3 py-2 border-b">เวลาทำ</th>
                  <th className="px-3 py-2 border-b">จัดการ</th>
                </tr>
              </thead>
              <tbody>
                {items.map((m) => (
                  <tr key={m.id} className="align-top">
                    <td className="px-3 py-2 border-b break-words">{m.nameTH}</td>
                    ...
                    <td className="px-3 py-2 border-b">
                      <div className="flex flex-wrap gap-1">
                        {m.tags.map((t) => (
                          <span key={t} className="px-2 py-0.5 rounded border text-xs bg-gray-50">{t}</span>
                        ))}
                      </div>
                    </td>
                    <td className="px-3 py-2 border-b">{m.budget ? `฿${m.budget}` : "-"}</td>
                    <td className="px-3 py-2 border-b">{m.cookMins ? `${m.cookMins} นาที` : "-"}</td>
                    <td className="px-3 py-2 border-b">
                      <div className="flex gap-2">
                        <button
                        ...
                        <button
                          onClick={() => remove(m.id)}
                          className="px-3 py-1 rounded border text-rose-600"
                          type="button"
                        >
                          Delete
                        </button>
        ...
        <div className="flex items-center justify-between p-3">
            ...
            {/* ปุ่มตัวเลขแบบย่อ: แสดงรอบ ๆ หน้า */}
          ...
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4">
            <div className="w-full max-w-lg rounded bg-white p-4 space-y-3">
                  ...
                  {(["breakfast", "lunch", "dinner"] as TimeKey[]).map((t) => (
                    <button
                      ...
                      className={`px-3 py-1 rounded border ${
                        editing.timeOfDay.includes(t) ? "bg-black text-white" : "bg-white"
                      }`}
                    >
                      {timeTH(t)}
                    </button>
                ...
                <div className="flex flex-wrap gap-2">
                  {allTags.map((t) => (
                    <button
                      ...
                      className={`px-3 py-1 rounded border ${
                        editing.tags.includes(t) ? "bg-black text-white" : "bg-white"
                      }`}
                    >
                      {t}
                    </button>
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button onClick={() => setEditing(null)} className="px-4 py-2 rounded border" type="button">ยกเลิก</button>
                <button onClick={saveEdit} className="px-4 py-2 rounded bg-gray-900 text-white" type="button">บันทึก</button>
              </div>
            </div>
          </div>
        )}
